...
class LoadImages:  # for inference
    def __init__(self, path, img_size=416,auto = False):
        path = str(Path(path))  # os-agnostic   资源地址
        files = []
        ...
        videos = [x for x in files if os.path.splitext(x)[-1].lower() in vid_formats] # 获取视频文件的地址
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video'
            ret_val, img0 = self.cap.read()# 此处开始读视频数据
            if not ret_val:# 决定是否结束读取
                self.count += 1
                self.cap.release()
        ...
        # Padded resize
        img = letterbox(img = img0, new_shape=self.img_size,auto = self.auto)[0]# resize
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.n = n
        # 第几个batch
        self.batch = bi  # batch index of image，图片的batch索引，代表第几个batch的图片
        self.img_size = img_size
        ...
        # Define labels，标签文件是通过images替换为labels, .jpg替换为.txt得到的
        self.label_files = [x.replace('images', 'labels').replace(os.path.splitext(x)[-1], '.txt')
                            for x in self.img_files]

        # Rectangular Training  https://github.com/ultralytics/yolov3/issues/232，矩形训练，训练的时候情况比较复杂。
        # 由于在训练过程中是一个batch的图片，而每个batch图片是有可能长宽比不同的，这就是与测试最大的区别。
        # 实现是取这个batch中最大的长和宽，然后将整个batch填充到max width和max height,这样操作对小一些的图片来说也是比较浪费
        # 这里的yolov3的实现主要是优化了一下如何将比例相近的图片放在一个batch，这样显然填充的就更少一些了。
        # 作者在issue中提到，在coco数据集中使用这个策略进行训练，能够快1/3。
        if self.rect:
            # Read image shapes (wh)，读取长和宽
            sp = path.replace('.txt', '.shapes')  # shapefile path
            try:
                with open(sp, 'r') as f:  # read existing shapefile，读取shape文件
                    s = [x.split() for x in f.read().splitlines()]
                    assert len(s) == n, 'Shapefile out of sync'
            except:
                # 如果没有.shape文件，就自己生成一个
                s = [exif_size(Image.open(f)) for f in tqdm(self.img_files, desc='Reading image shapes')]
                np.savetxt(sp, s, fmt='%g')  # overwrites existing (if any)，如果存在也会重写

            # Sort by aspect ratio，根据长宽比进行排序
            ...
            # Set training image shapes，设置训练的图片形状
            shapes = [[1, 1]] * nb
            for i in range(nb):
                ari = ar[bi == i]
        ...
        # 预载标签(weighted CE 训练时需要这个步骤)，否则无法按照权重进行采样
        self.imgs = [None] * n
        self.labels = [None] * n
        if cache_labels or image_weights:  # cache labels for faster training
            ...
            for i, file in enumerate(pbar):
                ...
                if l.shape[0]:
                    ...
                    # Create subdataset (a smaller dataset)，创建一个小型的数据集进行试验
                    ...
                    # 为两阶段分类器提取目标检测的检测框，默认开关是关掉的，不是很理解
        ...
        if cache_images:  # if training
            gb = 0  # Gigabytes of cached images，计算缓存到内存中的图片占用的空间GB为单位
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if os.path.isfile(label_path):
                x = self.labels[index]
                if x is None:  # labels not preloaded，如果标签没有加载，读取label_path内容
                    with open(label_path, 'r') as f:
                        x = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)

                if x.size > 0:
                    # Normalized xywh to pixel xyxy format，将归一化后的xywh转化为左上角、右下角的表达形式
                    labels = x.copy()
        ...
        nL = len(labels)  # number of labels，标注文件个数
        if nL:
            # convert xyxy to xywh，将 xyxy 格式转化为 xywh 格式
            labels[:, 1:5] = xyxy2xywh(labels[:, 1:5])

            # Normalize coordinates 0 - 1，归一化到0-1之间
